...
if __name__ == "__main__":
    ...
    abnormalities_data = pd.read_csv(
        args.nlst_abnormalities_csv, low_memory=True)
    ...
    for row_dict in tqdm(source_json):
        ...
        img_posn = float(
            literal_eval(row_dict["dicom_metadata"]
        ...
        if pid in pid_list:
            ...
            if (len(pid_list) > 0) and (len(pid_list) % 500 == 0):
                elapsed = round((time.time() - now) / 3600, 3)
                print(
                    "Processed {} participants in {} hrs".format(
                        len(pid_list), elapsed)
                )
            pid_list.append(pid)
