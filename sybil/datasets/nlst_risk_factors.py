...
class NLSTRiskFactorVectorizer:
    def __init__(self, args):
        ...
        self.risk_factor_transformers = {
            ...
            "years_since_quit_smoking": self.get_years_since_quit_smoking_transformer(
                "years_since_quit_smoking", [0, 10]
            ),
        }
        ...
        for k in self.risk_factor_keys:
            if k not in self.risk_factor_transformers.keys():
                raise Exception(
                    "Risk factor key '{}' not supported.".format(k))
    ...
    def get_age_risk_factor_transformer(self, risk_factor_key, cutoffs):
        def transform_age_risk_factor(
            patient_factors, screen_timepoint, just_return_feature_names=False
        ):
            ...
            days_since_randomization = patient_factors[
    ...
    def transform_binary_family_history(
        self, patient_factors, screen_timepoint, just_return_feature_names=False
    ):
        if just_return_feature_names:
        ...
        risk_factor = int(
            any(
                [
                    patient_factors[key][0] == 1
                    for key in patient_factors
    ...
    def transform_smoking_tails(self, risk_factor_key, cutoffs):
        def transform_smoking_risk_factors(
            patient_factors, screen_timepoint, just_return_feature_names=False
        ):
    ...
    def get_years_since_quit_smoking_transformer(self, risk_factor_key, cutoffs):
        def transform_exam_one_hot_risk_factor(
            ...
            years_since_quit_smoking = (
                0 if is_smoker else current_age - age_quit_smoking
            )
            binary_risk_factor = torch.zeros(2)
    ...
    def get_binary_transformer(self, risk_factor_key):
        def transform_binary(
            ...
            if just_return_feature_names:
                return [
                    "no_{}".format(risk_factor_key),
                    "has_{}".format(risk_factor_key),
                ]
            binary_risk_factor = torch.zeros(2)
            risk_factor = int(patient_factors[risk_factor_key][0])
    ...
    def transform(self, patient_factors, screen_timepoint):
        risk_factor_vecs = [
            self.risk_factor_transformers[key](
                patient_factors, screen_timepoint)
