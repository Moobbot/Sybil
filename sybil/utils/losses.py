    ...
    for attn_num in [1, 2]:
        ...
        if model_output.get("image_attention_{}".format(attn_num), None) is not None:
            if len(batch["image_annotations"].shape) == 4:
                batch["image_annotations"] = batch["image_annotations"].unsqueeze(
                    1)
            ...
            annotation_gold = annotation_gold * \
                batch_mask[:, None, None, None, None]

            # renormalize scores
            mask_area = annotation_gold.sum(
            ...
            num_annotated_samples = (
                annotation_gold.view(B * N, -1).sum(-1) > 0).sum()
            ...
            logging_dict["image_attention_loss_{}".format(
                attn_num)] = loss.detach()
            ...
            loss = (
                F.cross_entropy(
                    side_attn_log, cancer_side_gold, reduction="none")
                * cancer_side_mask
        ...
        if model_output.get("volume_attention_{}".format(attn_num), None) is not None:
            ...
            annotation_gold = batch["annotation_areas"].float(
            ) * batch_mask[:, None]
            ...
            logging_dict["volume_attention_loss_{}".format(
                attn_num)] = loss.detach()
