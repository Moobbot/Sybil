...
class Simple_AttentionPool(nn.Module):
    ...
    def forward(self, x):
        ...
        output["volume_attention"] = self.logsoftmax(
            attention_scores.transpose(1, 2)
        ).view(B, -1)
        attention_scores = self.softmax(
            attention_scores.transpose(1, 2))  # B, 1, N
...
class Simple_AttentionPool_MultiImg(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_scores = self.softmax(
            attention_scores.transpose(1, 2))  # BT, 1, WH

        x = x * attention_scores  # BT, C, WH
        x = torch.sum(x, dim=-1)
        output["multi_image_hidden"] = x.view(
            B, T, C).permute([0, 2, 1]).contiguous()
